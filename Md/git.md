...
仮に、ファイルの変更履歴をアナログなやり方で管理するとなると大体以下の様になる。
- ファイルの名称に"ver"だとか"rev"とかをつけてナンバリング
- バージョン管理用の台帳でも作る  
  
しかし、これだとバージョンの前後で具体的にどこを変更したか探すのが大変であり、数が増えてくるとどのバージョンが何の変更を何のためにしたものか正直わからない。(台帳に全部書けばいい話だけど、それも煩雑)  
さらにバージョンを戻すときは、一々コピペしてファイル名変えて、今はどのバージョンを使っているかメモして...なんて事をしなければならない。  

gitはこれらをもっと楽に実現しているといったイメージのものである。

### Gitによる履歴管理
gitの管理対象とするディレクトリには「.git」フォルダが作られ、これが履歴管理用のローカルリポジトリ(履歴情報の保管庫)となる。(図１参照)  
|![](./pic/git1.jpg)|
|:---:|
| 図１　.gitディレクトリ|  

gitはローカル内に大きく分けて３つのファイル保存領域(リモートは数に入れてない)を持ち、管理対象のファイルは以下の手順で状態遷移・登録される。
```
