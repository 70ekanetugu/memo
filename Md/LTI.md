...
- JSONスキーマ
    - 基本的なLTIメッセージのペイロードの形式を定義するjsonスキーマ。(jsonのバリデーションに使える)
- LtiResourceLinkRequest.json

### LTIドメインモデル  
[IMS公式](https://www.imsglobal.org/spec/lti/v1p3/#platforms-and-tools)の3.1.2項参照。  
以降で説明するパラメータが何のパラメータなのか一目でわかる。

### 主要な概念・要素
1. プラットフォーム  
従来のToolConsumerの事。(学習ツール)また、OpenIdのプロバイダーとしても機能する。

1. ツール  
従来のToolProvierの事。LMS  
...
1. ツールの展開  
"ツールの展開"はツールが使用可能なコンテキストの範囲を定義する。  
プラットフォーム(LMS)は展開されたツールに一意なdeployment_idを生成する。ツールのインスタンスみたいなもの。  
対して、ツールそのものの識別子は"client_id"であり、これはツールごとに一意であり且つただ１つである。

1. LTIリンク  
プラットフォームに保存されている、特定のツールへの参照である。
LTIリンクのmessage_typeに依存するリソースを提供するツール。  
LTIリンクは、ツールの指すURLと、各リンク固有の識別及び表示目的で使用される他のメタデータが含まれている。  
...
Line itemを配列として持っているもの。DBのテーブルと思ってよい。
1. Result
プラットフォーム採点簿のセル。つまりあるLine itemの値。指定のユーザーとline itemに一意である。  
値としては、数値型のSCOREとコメントを持つ。  
...
以下に上図３を元にした実フロー解説を記載する。(項番は図と連動)

##### 1. ajaxリクエスト/レスポンス  
=> ユーザーがプラットフォーム上でツールの起動を行った時(コースの受講、URL押下など)、LMSからパラメータの取得を行う。(今回はajaxとして話を進める)
このajaxレスポンスは、各LTIパラメータがセットされたhiddenなinput要素を含むForm要素である。
レスポンスの読み込み後(Formロード後)、javascriptのsubmit()で擬似的なPOSTリダイレクトを行う。(=>2. ログインリクエストへ)  
...
前述で取得したDBレコードにある「LMSの認証URL」のクエリーストリングに上記パラメータを
...
id_tokenはユーザーの識別子、Launchレクエストをする為にツールに必要なメッセージclaimsを含める必要がある。
このレスポンスは以下のパラメータをセットして、redirect_uriに送られる。
...
そして、ツール(redirect_uri)へ向けてjavascriptでPOST擬似リダイレクト(submit())する。(8.認証レスポンスへ)
...
- JWTをBase64でデコード
- LMSから公開鍵を取得
  - "ツールからLMSへ"GETリクエストを行う。
  - リクエスト先URLは、DBに事前登録したURL(公開鍵リクエスト先)
  - デコードしたJWTヘッダのKidをクエリーストリングとしてリクエストに付ける  

検証が問題なければ、リソース(target_link_uri)へリダイレクトさせる。(10.リソースの表示へ)

##### 10. リソースの表示  
=> target_link_uriへリダイレクトさせる。


##### 認証エラー応答
これもOpenId Connectで仕様が決められている。
...
プラットフォームから送られてくるJWTは正確には以下の様になるが、ほとんどがオプションである為、とりあえずは前述の必須項目を検証するようにしておけば仕様上は問題なし。
...
この機能(サービス)のインスタンスをLIS(Learning Information Service)という。  
...
LTI起動メッセージで送られてくるJWTに、同コンテキスト内で許可されている権限及びLISサービスのエンドポイントURLが入っている。  
以下の様な感じ。
...
詳細は後述するが、サービスリクエスト時のJWTはLineitemかLineitemsかによって以下の通り。
```json
...
##### 3. Resultサービス
結果サービスを利用すると、ツールはプラットフォーム独自の項目について結果を紹介できる。結果は、プラットフォームの成績表にある特定のユーザの現在の成績を表す。(LMS側の成績をツール側で使いたいときに使うサービス)  
ツールがプラットフォーム内の成績を直接変更するべきではない為、GETリクエストしかできない。
※Scoreと対のサービスの為、今回は省略。

##### 4. Scoreサービス
ツール内特定のLine itemのユーザーステータスを表す。ツールでの受講結果の事。  
